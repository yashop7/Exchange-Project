                ...
                <div className="flex flex-col px-3">
                    <div className="flex flex-col flex-1 gap-3 text-baseTextHighEmphasis">
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center justify-between flex-row">
                                <p className="text-base  text-slate-400 font-normal ">Available Balance</p>
                                <p className="font-medium text-base text-baseTextHighEmphasis ">36.94 USDC</p>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex justify-end flex-row">
                            <p className="font-medium pr-2 text-md text-baseTextMedEmphasis">≈ 0.00 USDC</p>
                        </div>
                    ...
                    <button type="button" className="font-semibold    focus:ring-blue-200 focus:none focus:outline-none text-center h-12 rounded-xl text-xl px-4 py-3 my-4 bg-[#00c177] text-gray-900 hover:bg-[#00c177]/80 cursor-pointer     active:scale-98" data-rac="">Buy</button>
                    <div className="flex justify-between flex-row mt-1">
                        <div className="flex flex-row gap-2">
                            <div className="flex items-center">
                                ...
                                <label className="ml-2 text-xs text-slate-400">Post Only</label>
                            </div>
                            <div className="flex items-center">
                                ...
                                <label className="ml-2 text-xs text-slate-400">IOC</label>
                            </div>
                        </div>
                </div>
            </div>
...
function LimitButton({ type, setType }: { type: string, setType: any }) {
    ...
    <div className={`text-base  font-medium py-1 border-b-2 ${type === 'limit' ? "border-accentBlue text-white" : "border-transparent text-slate-400 hover:border-baseTextHighEmphasis hover:text-baseTextHighEmphasis"}`}>
        Limit
    </div>
</div>
}
...
function MarketButton({ type, setType }: { type: string, setType: any }) {
    ...
    <div className={`text-base  font-medium py-1 border-b-2 ${type === 'market' ? "border-accentBlue text-white" : "border-b-2 border-transparent text-slate-400 hover:border-baseTextHighEmphasis hover:text-baseTextHighEmphasis"} `}>
        Market
    </div>
    </div>
}
...
function BuyButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: any }) {
    return <div className={`flex flex-col mb-[-2px] flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'buy' ? 'border-b-greenBorder bg-greenBackgroundTransparent' : ' hover:border-b-neutral-600 border-b-neutral-800 '}`} onClick={() => setActiveTab('buy')}>
...
function SellButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: any }) {
    return <div className={`flex flex-col mb-[-2px] flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'sell' ? 'border-b-redBorder bg-redBackgroundTransparent' : ' hover:border-b-neutral-600 border-b-neutral-800 '}`} onClick={() => setActiveTab('sell')}>
